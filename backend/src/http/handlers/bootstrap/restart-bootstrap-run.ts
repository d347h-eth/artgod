...
import type { RestartBootstrapRunInput } from "../../../application/use-cases/bootstrap/restart-bootstrap-run.js";
import type { CreateBootstrapRunOutput } from "../../../application/use-cases/bootstrap/types.js";

export type RestartBootstrapRunRoute = {
    Params: {
        chain_ref: string;
...
export class RestartBootstrapRunHttpAdapter {
    constructor(
        private readonly restartBootstrapRunPort: {
            restartRun(
    ...
    readonly handle = async (
        request: FastifyRequest<RestartBootstrapRunRoute>,
    ) => {
        const input = this.mapRequestToInput(request);
        const output = await this.restartBootstrapRunPort.restartRun(input);
        return output;
    };

    private mapRequestToInput(
        request: FastifyRequest<RestartBootstrapRunRoute>,
    ): RestartBootstrapRunInput {
...
function parseManualInput(value: unknown): RestartBootstrapRunInput["manualInput"] {

...
describe("get collection activity span attributes", () => {
    it("summarizes activity request shape without raw filter values", () => {
        const attributes = getCollectionActivitySpanAttributes(
            request(
                `/api/ethereum/terraforms/activity?limit=250&kind=sales&cursor=opaque&traits=Hat:Beanie,Mood:Calm&trait_ranges=Power:3..9&token_id=1&maker=0xabc&content_hash=0xhash&event_group=dream&${COLLECTION_MEDIA_QUERY_PARAMS.MediaMode}=${COLLECTION_MEDIA_MODES.Snapshot}`,
            ),
        );
    ...
    it("marks extension event feeds separately from grouped kind feeds", () => {
        ...
        expect(attributes).toMatchObject({
            [ARTGOD_SPAN_ATTRIBUTE.ActivityKind]:
                ARTGOD_TRACE_ATTRIBUTE_VALUE.None,
            [ARTGOD_SPAN_ATTRIBUTE.ActivityExtensionEvent]: "terraforms:beacon",

...
const OUTPUT_WINDOW_SIZE_PATH: &str = "generated_window_size.rs";
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let tauri_config_path = manifest_dir.join(TAURI_CONFIG_PATH);
    let out_dir = PathBuf::from(env::var("OUT_DIR")?);
    let output_path = out_dir.join(OUTPUT_FONT_PATH);
    let window_size_output_path = out_dir.join(OUTPUT_WINDOW_SIZE_PATH);
    ...
    fs::write(
        window_size_output_path,
        render_admin_window_size(&tauri_config_path)?,
    )?;
    Ok(())
}

fn render_admin_window_size(config_path: &Path) -> Result<String, Box<dyn Error>> {
    let source = fs::read_to_string(config_path)?;
    let config: serde_json::Value = serde_json::from_str(&source)?;
    let window = config
        .pointer("/app/windows/0")
        .ok_or_else(|| invalid_config("Tauri config is missing app.windows[0]"))?;
    let width = window
        .get("width")
        .and_then(serde_json::Value::as_u64)
        .and_then(|value| u32::try_from(value).ok())
        .ok_or_else(|| invalid_config("Tauri Admin window width must be a positive u32"))?;
    let height = window
        .get("height")
        .and_then(serde_json::Value::as_u64)
        .and_then(|value| u32::try_from(value).ok())
        .ok_or_else(|| invalid_config("Tauri Admin window height must be a positive u32"))?;
    if width == 0 || height == 0 {
        return Err(invalid_config("Tauri Admin window dimensions must be positive").into());
    }
    Ok(format!(
        "// Generated by build.rs from {TAURI_CONFIG_PATH}.\npub const ADMIN_WINDOW_SIZE: (u32, u32) = ({width}, {height});\n"
    ))
}

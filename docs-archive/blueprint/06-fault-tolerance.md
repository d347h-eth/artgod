...
2.  **Rollback:** For every block > Fork Point:
    ...
    - Delete `activities`.
    - _Implementation:_ Uses `removeEvents` methods in storage adapter which executes `DELETE` or inverse `UPDATE` queries.
...
- **Mechanism:**
    1.  `SyncWorker` does _not_ write to `nft_balances`.

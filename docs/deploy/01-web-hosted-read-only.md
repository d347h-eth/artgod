...
- `RPC_URL_LIST=[{"url":"http://ethereum-rpc:8545","weight":1}]`
- `RPC_WS_URL_LIST=[{"url":"ws://ethereum-rpc:8546","weight":1}]`
...
`RPC_URL_LIST` is a weighted endpoint pool; for one same-host node, use `[{"url":"http://ethereum-rpc:8545","weight":1}]`.
...
- `RPC_URL_LIST=[{"url":"http://ethereum-rpc:8545","weight":1}]`
- `RPC_WS_URL_LIST=[{"url":"ws://ethereum-rpc:8546","weight":1}]` if using the node WebSocket endpoint
- `OPENSEA_API_KEY`
...
- In same-host reth deployments, use `RPC_URL_LIST=[{"url":"http://ethereum-rpc:8545","weight":1}]` and `RPC_WS_URL_LIST=[{"url":"ws://ethereum-rpc:8546","weight":1}]`; the reth container must join `ETHEREUM_RPC_NETWORK` with the `ethereum-rpc` alias.
...
- The deploy image relies on the repo’s Yarn allowlist policy during `yarn install --immutable --inline-builds`: `enableScripts: false` stays in effect globally, while allowlisted packages such as `esbuild` are still built through `dependenciesMeta.built: true`. `better-sqlite3` is then built explicitly and narrowly through `yarn build:sqlite-native`, which invokes its trusted package-local `install` script from inside the unplugged package directory and hard-fails if the native SQLite binding is still missing from `.yarn/unplugged`.
- The deploy image keeps `sharp` external to the runtime bundle so Yarn PnP resolves it from the indexer package boundary and its optional `@img/*` native packages remain visible. After `yarn build:runtime`, the image runs `scripts/build/check-native-runtime-dependencies.mjs` to require `better-sqlite3` and `sharp` from runtime-artifact issuers before the final image is assembled.
- The deploy image and desktop build reuse the same backend/indexer entrypoints
  and artifact builder, but select different release profiles: deploy retains
  the full observability graph and Yarn PnP, while desktop keeps
  backend/indexer metrics and every APM/profile path on no-op adapters, admits
  only the narrow loopback trading Prometheus facade, and uses isolated
  package-local native dependencies.

...
- The deploy image and desktop build reuse the same backend/indexer entrypoints and artifact builder, but select different release profiles: deploy retains observability and Yarn PnP, while desktop selects no-op observability adapters and isolated package-local native dependencies.

...
| Backend   | backend API                        | Bootstrap contract probe                                               | `backend/src/infra/bootstrap/viem-bootstrap-contract-probe.ts` | `RPC_URL_LIST`                            | `backend-rpc`                                                                        | Yes           | Yes             | Yes        | Pre-bootstrap ERC721 probing reads ERC165 support, supply, first token, token URI, and owner fallback through the resilient backend RPC client. Missing methods and reverts are deterministic contract failures, so they do not exhaust retries. Token URI payload and image-size fetches are ordinary HTTP/media fetches, not JSON-RPC. |

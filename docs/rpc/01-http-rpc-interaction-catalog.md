...
| Backend   | backend API                        | Bootstrap contract probe                                               | `backend/src/infra/bootstrap/viem-bootstrap-contract-probe.ts` | `RPC_URL_LIST`                            | `backend-rpc`                                                                        | Yes           | Yes             | Yes        | Pre-bootstrap ERC721 probing reads ERC165 support, collection name, supply, sample token, owner validation, and token URI through the resilient backend RPC client. Missing methods and reverts are deterministic contract failures, so they do not exhaust retries. Token URI payload and image-size fetches are ordinary HTTP/media fetches, not JSON-RPC. |
...
- Concrete RPC adapter methods: `ViemBackendRpcClient.readContract`,
  `getBytecode`, and `getStorageAt`.
- RPC method path: `getBytecode` preflight, optional ERC-1967 implementation and
...
- Proxy handling: recognized EIP-1167 minimal proxy bytecode and ERC-1967
  implementation/beacon storage slots are reported with deterministic
  implementation identity, while contract reads continue to target the
...
This retained outcome is tracked as `BKL-056` in the

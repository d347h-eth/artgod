...
The runtime env defaults are sourced from `config/settings.manifest.toml` and generated into `.env.example`. Docker/container listeners are declared in the compose and observability config files listed below.

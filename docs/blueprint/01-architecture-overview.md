...
*   **Domain (Core):** Contains business logic, entities, and use cases. It knows *nothing* about the database, queue, or API.
    *   *Entities:* `Block`, `Transaction`, `Log`, `Order`, `Token`, `Collection`.
    *   *Ports (Interfaces):* `BlockRepository`, `JobQueue`, `RpcProvider`, `CacheService`.
*   **Application (Use Cases):** Orchestrates the flow of data.
    *   *Use Cases:* `SyncBlock`, `ProcessLog`, `IngestOrder`, `BackfillRange`.
*   **Infrastructure (Adapters):** Implementations of the ports.
    *   *Adapters:* `PostgresBlockRepository`, `RabbitMqJobQueue`, `AlchemyRpcProvider`, `RedisCacheService`.

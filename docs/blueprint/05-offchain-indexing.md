...
2.  **Validate:**
    ...
    *   **Conduit Check:** Verify if the maker has approved the OpenSea Conduit.
        *   *Optimization:* Cache "Open Channels" in DB (`seaport_conduit_open_channels`).
        *   *Fallback:* If channel unknown, query `ConduitController` contract on-chain.
...
*   **Filter Gate:** In `SyncPipeline`, after `ParseEvents`, strictly filter `EnhancedEvents`.
    *   *Rule:* Keep event IF `address == FOCUS_CONTRACT` OR `txHash` contains a relevant event.

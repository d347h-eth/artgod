...
- **`nft_transfer_events`**: Immutable log.
    ...
    - _Constraint:_ Unique on `(tx_hash, log_index, batch_index)`.
- **`nft_balances`**: Current state.
    - `contract`, `token_id`, `owner`, `amount`.
    - _Update Logic:_ Upsert. `amount = amount + delta`.
    - _Optimization:_ During backfill, writes are serialized to avoid row locking contention.
...
- **`fills`**: Sales history.
    ...
    - _Logic:_ When a fill event occurs, the associated order in `orders` must be updated to `filled` (or partial fill quantity updated).
...
- **Downstream Action:** A job `OrderUpdatesByMaker` picks this up, queries _all_ active orders for this user/token, and re-validates them (checks balance/allowance). If validation fails, order status -> `no-balance`.

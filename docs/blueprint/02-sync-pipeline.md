...
1.  **Realtime:**
    ...
    *   Publish `SyncJob(block: N)` to `realtime-queue`.
    *   *Optimization:* Cache `eth_getBlock` result in Redis immediately to save RPC calls for workers.
2.  **Backfill:**
...
*   **Optimization:**
    *   *Realtime:* Check Redis cache for block/tx data first.
    *   *Backfill:* Use specific "Backfill RPC" provider if configured (lower rate limits, higher latency ok).
...
*   **Strategy:**
    *   *Realtime:* Write directly to tables (`nft_transfer_events`, `orders`, `fills`).
    *   *Backfill (High Load):* Use **Write Buffers**. Push data to a `write-buffer-queue`. A dedicated serial worker drains this queue to Postgres to prevent deadlocks (locking `nft_balances` is expensive).
    *   *Focus Mode:* If configured, filter `OnChainData` to only include the target collection *before* writing.

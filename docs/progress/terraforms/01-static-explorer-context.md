...
- Each level now receives a transient surface assignment on page load: a random Zone palette available on that level plus a generated Perlin heightmap sampled at that level's exact contract dimensions. The renderer draws those cells as real `PlaneView.TOP` isometric rectangles instead of an SVG pattern, so texture density and cell count track the exact contract parcel area for that level. This surface texture is separate from the level topography values used for Zone attribution.

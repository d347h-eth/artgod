...
class ConsoleStorageWriter implements StorageWriter {
  async writeBatch(data: TokenData[]): Promise<void> {
    ...
    if (successfulTokens.length > 0) {
      console.log(`✅ ${successfulTokens.length} successful tokens: ${successfulTokens.map(t => t.tokenId).join(', ')}`);
      // Show sample of tokenURI data size
      if (successfulTokens.length > 0) {
        const sampleSize = successfulTokens[0].tokenURI.length;
        console.log(`📊 Sample tokenURI size: ${sampleSize.toLocaleString()} characters`);
      }
    }
...
class TerraformDataFetcher {
  ...
  private createDirectRequest(tokenId: number): { jsonrpc: string; method: string; params: any[]; id: number } {
    return {
      jsonrpc: '2.0',
      method: 'eth_call',
      params: [
  ...
  private createMulticallRequest(tokenIds: number[]): { jsonrpc: string; method: string; params: any[]; id: number } {
  ...
  private processMulticallResponse(response: any, tokenIds: number[]): TokenData[] {
    try {
      if (response.error) {
  ...
  private createTokenBatches(): number[][] {
    ...
    for (let tokenId = MIN_TOKEN_ID; tokenId <= MAX_TOKEN_ID; tokenId += TOKENS_PER_MULTICALL) {
      const batch: number[] = [];
      for (let i = 0; i < TOKENS_PER_MULTICALL && tokenId + i <= MAX_TOKEN_ID; i++) {
  ...
  async fetchAllTokenDataDirect(): Promise<void> {
    ...
    for (let i = 0; i < tokenList.length; i += POST_BATCH_SIZE) {
      ...
      console.log(`Estimated gas requirement: ${(tokenBatch.length * ESTIMATED_GAS_PER_TOKEN).toLocaleString()} gas`);
      console.log(`Token range: [${tokenBatch[0]}-${tokenBatch[tokenBatch.length-1]}]`);
      ...
      try {
        ...
        console.log(`Sending ${rpcRequests.length} direct tokenURI calls in single JSON-RPC batch...`);
        ...
        for (let j = 0; j < responses.length; j++) {
          ...
          const tokenData = this.processDirectResponse(response, tokenId);
        ...
        console.log(`Success rate: ${((successCount / tokenBatch.length) * 100).toFixed(1)}%`);
        console.log(`Effective throughput: ${(successCount / (totalTime / 1000)).toFixed(2)} successful tokens/second`);
        console.log(`Total throughput: ${(tokenBatch.length / (totalTime / 1000)).toFixed(2)} tokens/second (including failures)`);
        console.log(`Time breakdown: Network+Node: ${networkTime}ms (${((networkTime/totalTime)*100).toFixed(1)}%), Decoding: ${decodeTime}ms (${((decodeTime/totalTime)*100).toFixed(1)}%)`);
        ...
        const errorTokenData: TokenData[] = tokenBatch.map(tokenId => ({
          tokenId,
          tokenURI: '',
          success: false,
  ...
  async fetchAllTokenData(): Promise<void> {
    ...
    console.log(`Created ${tokenBatches.length} token batches of ${TOKENS_PER_MULTICALL} tokens each`);

    const totalBatchGroups = Math.ceil(tokenBatches.length / POST_BATCH_SIZE);
    const batchesToProcess = TEST_MODE ? 1 : totalBatchGroups;
    
    console.log(`${TEST_MODE ? 'TEST MODE: ' : ''}Will process ${batchesToProcess} batch group(s) of ${totalBatchGroups} total`);

    // Process batches in groups (double batching)
    for (let i = 0; i < tokenBatches.length && i / POST_BATCH_SIZE < batchesToProcess; i += POST_BATCH_SIZE) {
      const batchGroup = tokenBatches.slice(i, i + POST_BATCH_SIZE);
      const batchNumber = Math.floor(i / POST_BATCH_SIZE) + 1;
      const totalTokensInBatch = batchGroup.reduce((sum, batch) => sum + batch.length, 0);
      ...
      console.log(`Estimated gas requirement: ${(totalTokensInBatch * ESTIMATED_GAS_PER_TOKEN).toLocaleString()} gas`);
      console.log(`Token ranges: ${batchGroup.map(batch => `[${batch[0]}-${batch[batch.length-1]}]`).join(', ')}`);
      ...
      try {
        const startTime = Date.now();
        console.log(`Sending ${rpcRequests.length} multicall requests in single JSON-RPC batch...`);
        ...
        for (let j = 0; j < responses.length; j++) {
          ...
          const tokenData = this.processMulticallResponse(response, tokenIds);
          allTokenData.push(...tokenData);
        ...
        console.log(`Total processing time: ${totalTime}ms`);
        console.log(`Results: ${successCount} successful, ${errorCount} errors`);
        console.log(`Success rate: ${((successCount / totalTokensInBatch) * 100).toFixed(1)}%`);
        console.log(`Effective throughput: ${(successCount / (totalTime / 1000)).toFixed(2)} successful tokens/second`);
        console.log(`Total throughput: ${(totalTokensInBatch / (totalTime / 1000)).toFixed(2)} tokens/second (including failures)`);
        console.log(`Time breakdown: Network+Node: ${networkTime}ms (${((networkTime/totalTime)*100).toFixed(1)}%), Decoding: ${decodeTime}ms (${((decodeTime/totalTime)*100).toFixed(1)}%)`);
        ...
        const errorTokenData: TokenData[] = batchGroup.flat().map(tokenId => ({
          tokenId,
          tokenURI: '',
          success: false,
          error: `Batch error: ${error instanceof Error ? error.message : 'Unknown'}`
...
async function main() {
  ...
  console.log(`Strategy: ${USE_MULTICALL ? 'Multicall3 Aggregation' : 'Direct Calls'}`);
  console.log(`POST batch size: ${POST_BATCH_SIZE} requests per batch`);
  
  if (USE_MULTICALL) {
    console.log(`Tokens per multicall: ${TOKENS_PER_MULTICALL}`);
    console.log(`Total tokens per batch: ${TOKENS_PER_MULTICALL * POST_BATCH_SIZE}`);
    console.log(`Estimated gas per multicall: ${(TOKENS_PER_MULTICALL * ESTIMATED_GAS_PER_TOKEN).toLocaleString()}`);
  } else {
    console.log(`Total tokens per batch: ${POST_BATCH_SIZE}`);
    console.log(`Estimated gas per call: ${ESTIMATED_GAS_PER_TOKEN.toLocaleString()}`);
  }
  
  console.log(`Test mode: ${TEST_MODE ? 'ON (single batch only)' : 'OFF (full range)'}`);
  console.log('========================\n');
